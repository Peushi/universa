...
export async function sendMessage(req, res) {
    ...
    try {
        ...
        const row = await db.get("SELECT data FROM worlds WHERE user_id = ?", [
            req.session.userId,
        ])
        const world = row ? JSON.parse(row.data) : null

        if (!world) {
            return res.status(404).json({ error: "No world found. Please generate a world first before starting a conversation" })
        }
